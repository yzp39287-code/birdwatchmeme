...
$("#emailForm").onsubmit=async e=>{e.preventDefault();const b=e.currentTarget.querySelector("button"),email=e.currentTarget.elements.email.value.trim();b.disabled=true;const {error}=await client.auth.signInWithOtp({email,options:{shouldCreateUser:true,emailRedirectTo:new URL(".",location.href).href}});b.disabled=false;if(error)return $("#loginMessage").textContent=error.message;$("#loginMessage").textContent="登录链接已发送，请打开邮件并点击 Sign in。"};
